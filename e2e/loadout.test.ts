...
import { addAssignmentToOrg, addChartToOrg, addEmployeeToOrg, addLoadoutToOrg, addRoleToOrg, addTruckToOrg, addUser, createOrganization, } from "@/api/firebase/firebaseService";
...
describe('Calibration Reports API Route E2E Tests', () => {
  ...
  test('should successfully create a new loadout', async () => {
    const newLoadout: Loadout = {
      ...
      productIds: [productA.productId, productB.productId]
    };
    
    const apiRoute = `${NEXT_PUBLIC_BASE_URL}/api/organizations/${testOrg1.organizationId}/loadouts`;

    // Create loadout via POST route
    ...
    const loadoutDoc = await adminDb.doc(`organizations/${testOrg1.organizationId}/loadouts/${newLoadout.loadoutId}`).get();
    expect(loadoutDoc.exists).toBe(true);
    expect(loadoutDoc.data()?.productIds).toEqual([productA.productId, productB.productId]);
  });

  // Test Case 2: Invalid Product ID Creation (POST)
  test('should fail to create a loadout if any productId does not exist', async () => {
    const invalidLoadout: Loadout = {
      loadoutId: "invalid-product-loadout",
      name: "Invalid Loadout",
      productIds: [productA.productId, "INVALID-PRODUCT-ID"] // Contains one valid and one invalid ID
    };
    ...
    const response = await fetch(apiRoute, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Cookie': `session-token=${adminUserAuthToken}` },
      body: JSON.stringify(invalidLoadout),
    });

    // Verify response if fail
  ...
  // Test Case 3: Successful Loadout Update (PUT)
  test('should successfully update an existing loadout', async () => {
    ...
    await addLoadoutToOrg(adminUserAuthToken, testOrg1.organizationId, initialLoadout);
    
    const apiRoute = `${NEXT_PUBLIC_BASE_URL}/api/organizations/${testOrg1.organizationId}/loadouts/${initialLoadout.loadoutId}`;
    ...
    const loadoutDoc = await adminDb.doc(`organizations/${testOrg1.organizationId}/loadouts/${initialLoadout.loadoutId}`).get();
    expect(loadoutDoc.data()?.name).toBe("Updated Loadout Name");
    expect(loadoutDoc.data()?.productIds).toEqual([productB.productId]);
  });

  // Test Case 5: Invalid Product ID Update (PUT)
  test('should fail to update a loadout if any new productId does not exist', async () => {
    const initialLoadout: Loadout = {
      loadoutId: "update-fail-loadout",
      name: "Initial Name",
      productIds: [productA.productId]
    };
    await addLoadoutToOrg(adminUserAuthToken, testOrg1.organizationId, initialLoadout);
    ...
    const updatedData = {
      ...initialLoadout,
      productIds: [productB.productId, "INVALID-PRODUCT-ID"] // Attempt to add an invalid product
    };
  ...
  // Test Case 6: Successful Loadout Deletion (DELETE)
  test('should successfully delete an existing loadout', async () => {
    ...
    await addLoadoutToOrg(adminUserAuthToken, testOrg1.organizationId, loadoutToDelete);
    
    const loadoutDocRef = adminDb.doc(`organizations/${testOrg1.organizationId}/loadouts/${loadoutToDelete.loadoutId}`);
    expect((await loadoutDocRef.get()).exists).toBe(true);

    const apiRoute = `${NEXT_PUBLIC_BASE_URL}/api/organizations/${testOrg1.organizationId}/loadouts/${loadoutToDelete.loadoutId}`;
  ...
  // Test Case 7: Business Logic Failure (Deleting a Loadout in Use)
  test('should fail with a 409 Conflict if deleting a loadout that is in use by an active assignment', async () => {
    // Make loadout and truck
    const loadoutInUse: Loadout = {
      loadoutId: "in-use-loadout",
      name: "Loadout In Use",
      productIds: [productA.productId]
    };
    const truckForAssignment: Truck = {
        name: "Test Truck",
        truckId: "TRUCK-01",
        tankType: TankType.SINGLE,
        chartId: 'chart-a',
    ...
    // Add loadout and truck to organization
    await addLoadoutToOrg(adminUserAuthToken, testOrg1.organizationId, loadoutInUse);
    await addTruckToOrg(adminUserAuthToken, testOrg1.organizationId, truckForAssignment);

    // Create an active assignment that links the user, truck, and loadout
    await addAssignmentToOrg(adminUserAuthToken, testOrg1.organizationId, {
      truckId: truckForAssignment.truckId,
      userId: adminAuthUser.uid,
      employeeId: "1",
      loadoutId: loadoutInUse.loadoutId, // This loadout is currently in use
    });

    // Attempt to delete the loadout that is now in use
    const apiRoute = `${NEXT_PUBLIC_BASE_URL}/api/organizations/${testOrg1.organizationId}/loadouts/${loadoutInUse.loadoutId}`;
    const response = await fetch(apiRoute, {
      method: 'DELETE',
      headers: { 'Cookie': `session-token=${adminUserAuthToken}` },
    });
    ...
    // Verify the loadout was NOT deleted from Firestore
    const loadoutDoc = await adminDb.doc(`organizations/${testOrg1.organizationId}/loadouts/${loadoutInUse.loadoutId}`).get();
    expect(loadoutDoc.exists).toBe(true);
  });
});
