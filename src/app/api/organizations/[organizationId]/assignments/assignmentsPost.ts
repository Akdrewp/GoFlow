...
export async function assignmentsPOST(
  ...
  try {
    ...
    const assignmentTruckIdSchema = assignmentSchema.pick({ truckId: true, employeeId: true, loadoutId: true });
    ...
    const userEmployeeProfile = await getEmployeeFromOrg(token, organizationId, initialAssignmentData.employeeId);
    
    // Check if user is not active
    if (!userEmployeeProfile.uid) {
      return NextResponse.json(
        { status: "fail", message: "Employee must have an active user account to assign" },
        { status: 400 } // Bad Request
      );
    }
    ...
    const assignmentData = {
      ...
      userId: userEmployeeProfile.uid,
    };
