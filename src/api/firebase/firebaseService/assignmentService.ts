...
export async function addAssignmentToOrg(token: string, organizationId: string, assignmentData: Omit<Assignment, "assignmentId" | "assignedAt" | "unassignedAt" >): 
  ...
  try {
    ...
    const userHasActiveAssignment = await assignmentDatabase.getFromUser(organizationId, assignmentData.userId);
    if(userHasActiveAssignment) {
      throw new FirebaseVerifyError(
        `Employee with ID "${assignmentData.employeeId}" already assigned`,
        409 // Conflict
      );
    }
 ...
 * @returns A promise that resolves to the user's active Assignment object, or undefined if none is found.
 * @throws {FirebaseVerifyError} If the requesting user does not have permission to view assignments.
 */
export async function getAssignmentFromUser(token: string, organizationId: string, userId: string): Promise<Assignment | undefined> {
  try {
    // Get active assignment from user
    const activeAssignment = await assignmentDatabase.getFromUser(organizationId, userId);

    // Check if there is an active assignment
    if (activeAssignment) {
      // Check if user is trying to access their own assignment
      const selfAssign = await userCanAccessAssignment(token, organizationId, activeAssignment?.assignmentId);
      if (!selfAssign) {
        // Not self access check general READ permission assignments
        const resourcePath = `organizations/${organizationId}/assignments`;
    ...
    console.error(`Error fetching assignment for user "${userId}":`, e);
    throw e;
  }
}
